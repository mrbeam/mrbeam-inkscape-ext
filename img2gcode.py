...
class ImageProcessor():
	...
	def __init__( self , feedrate = 500, contrast = 1.0, sharpening = 1.0, beam_diameter = 0.25, intensity_black = 1000, intensity_white = 0, pierce_time = 500, material = "default"):
		
		self.beam = beam_diameter
		self.pierce_time = "{0:.3f}".format(pierce_time/1000.0)
		...
		self.feedrate_white = 500
		self.feedrate_black = 50
	...
	def generate_gcode(self, img, x,y, mode = "feedrate"):
						...
						if(mode == "intensity"):
							if(lastBrightness >= 255): 
								...
								if(self.pierce_time > 0):
									gcode += "M3 S"+str(pierce_intensity)+ "\n" + "G4 P"+str(self.pierce_time)+"\n" # Dwell for P ms
...
if __name__ == "__main__":
	...
	print("###", options.pierce_time)
	ip = ImageProcessor(options.feedrate, options.contrast, options.sharpening, options.beam_diameter, options.intensity_black, options.intensity_white, options.pierce_time, material = "default")
	path = args[0]
